...
for idx = 1:1:4
    ...
    sp{idx} = load([movie_path,'\',sparse_name{idx}]);
end
...
fly_2d = sp{camvec(cam)}.frames(210+71).indIm;

pts = [fly,ones(size(fly,1),1)]

pt2d = cam_mat{cam}*pts'
pt2d = (pt2d./pt2d(3,:))'
figure
scatter(fly_2d(:,1),fly_2d(:,2));hold on
ewc = easyWandData.coefs
[uv] = dlt_inverse(ewc(:,camvec(cam)),fly)
scatter(uv(:,1),uv(:,2))
